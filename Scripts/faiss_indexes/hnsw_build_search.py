# Builds an HNSW FAISS index from existing isolate_embeddings.npy files.
...
def process_model(model_dir):
    ...
    hnsw_dir = model_dir / "faiss_indexes" / "hnsw"
    hnsw_dir.mkdir(parents=True, exist_ok=True)

    index_out = hnsw_dir / "hnsw.index"
    metadata_out = hnsw_dir / "hnsw_metadata.csv"
    neighbors_out = hnsw_dir / "hnsw_neighbors.csv"
    metrics_out = hnsw_dir / "hnsw_metrics.csv"
    runtime_out = hnsw_dir / "hnsw_runtime.csv"
